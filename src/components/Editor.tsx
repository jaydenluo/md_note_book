  ...
  useEffect(() => {
    if (note) {
      ...
      console.log("笔记内容更新:", {
        ...
        contentPreview: note.content?.substring(0, 200) || "空内容",
      });
      ...
      <>
        {items.map((item, index) => (
          <div key={`outline-item-${item.id || index}`}>
            <div className="flex items-center cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 p-1 rounded" style={{ paddingLeft: `${(item.level - 1) * 12}px` }}>
              {/* 折叠/展开图标，只有有子项时才显示 */}
              {item.children.length > 0 && (
                ...
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className={`h-3 w-3 transition-transform ${item.isCollapsed ? "" : "transform rotate-90"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                  </svg>
                </div>
              )}
              {/* 如果没有子项，添加一个空的占位符，保持对齐 */}
              {item.children.length === 0 && <div className="mr-2 w-4 h-4"></div>}
                ...
                onClick={() => {
                  ...
                  if (foundPos === null) {
                    editor.state.doc.descendants((node: any, pos: number) => {
                      if (node.type.name === "heading") {
                        // 比较标题文本 - 包含匹配
                        if (node.textContent && (node.textContent.includes(item.text) || item.text.includes(node.textContent))) {
                          foundPos = pos;
                          foundNode = node;
            ...
            {/* 递归渲染子项，根据折叠状态显示或隐藏 */}
            {item.children.length > 0 && !item.isCollapsed && <div className="pl-2">{renderOutlineTree(item.children)}</div>}
          </div>
        ))}
      </>
    );
          ...
          <div ref={outlinePanelRef} className="relative h-full bg-white dark:bg-gray-800 border-l border-gray-200 dark:border-gray-700 transition-all duration-300 shadow-sm" style={{ width: outlineWidthPercent }}>
            ...
            <div className="h-full overflow-y-auto p-4">
              <div className="flex justify-between items-center mb-4 pb-2 border-b border-gray-100 dark:border-gray-700">
                ...
                <button onClick={regenerateOutline} className="text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 outline-refresh-btn transition-transform duration-300" title="重新生成大纲">
              ...
              <div className="space-y-1 text-sm">{outlineItems.length > 0 ? renderOutlineTree(outlineItems) : <div className="text-gray-400 italic">无大纲项</div>}</div>
            </div>
          </div>
        )}
      </div>
